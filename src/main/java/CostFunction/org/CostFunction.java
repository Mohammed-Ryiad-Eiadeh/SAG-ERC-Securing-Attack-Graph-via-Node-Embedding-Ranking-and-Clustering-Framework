...
/**
 * This class is used to calculate the total cost that she loses if according to the given attack path
 */
public class CostFunction {
    private final double[][] adjMatrix;
    private final double[] assetsLossVec;

    /**
     * This is the default constructor of the CostFunction class
     * @param adjMatrix The graph representation as adjacency matrix
    ...
    public CostFunction(double[][] adjMatrix, double[] assetLoss) {
        ...
        this.assetsLossVec = assetLoss;
    }

    /**
     * This method is used to compute the cost
     * @param solution The attack path
    ...
    public double computeCost(List<Integer> solution) {
        ...
        for (int i = 0; i < solution.size() - 1; i++) {
            ...
            cost += Math.exp(-adjMatrix[from - 1][to - 1]) * assetsLossVec[to - 1];
        }
        return cost;
    }
}